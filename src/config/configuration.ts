export default () => ({
  ...
  plugins: {
    ...
    // Remote catalog of installable plugins (JSON array; the Zetu-plugins repo's plugins.json).
    // Fetched through the SSRF guard — add its host to SSRF_ALLOWED_HOSTS if it is not publicly resolvable.
    catalogUrl:
      process.env.PLUGIN_CATALOG_URL || 'https://raw.githubusercontent.com/learninghub44/Zetu-plugins/main/plugins.json',
